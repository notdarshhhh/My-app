      ...
      <div className='container' style={{ color: props.mode === "dark" ? "white" : "black" }}>
        ...
        <div className="mb-3">
          <textarea className="form-control" id="exampleFormControlTextarea1" rows="7" value={text} onChange={handleOnChange} style={{ background: props.mode === "dark" ? "#13466e" : "white", color: props.mode === "dark" ? "white" : "black" }}></textarea>
            ...
            <button disabled={text.length === 0} className="btn bt mx-2 my-2" onClick={handleUpClick} >Convert to Uppercase</button>
            <button disabled={text.length === 0} className="btn bt mx-2 my-2" onClick={handleLowClick}>Convert to Lowercase</button>
            <button disabled={text.length === 0} className="btn bt mx-2 my-2" onClick={handleClear}>Clear Text</button>

            <button disabled={text.length === 0} className="btn bt mx-2 my-2" onClick={handleCopy}>Copy Text</button>

            <br />
            <br />
            <input type="text" id='find' className='mx-2 my-2' />
            <input type="text" id='replace' className='mx-2 my-2' />
            <button disabled={text.length === 0} className="btn bt mx-2 my-2" onClick={handleReplace}>Replace Text</button>
          </div>
        </div>
